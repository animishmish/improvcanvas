...
function step1 () {
  ...
  navigator.getUserMedia({audio: true, video: true}, function(stream){
    ...
    // If video is available: Hide "Prompt" + "Close call"
    $('#step1, #step3').hide();
    $('#step2').show();
...
function receiveCall(call) {
  ...
  //if in a call: Hide Prompt + Call info
  $('#step1, #step2').hide();
  $('#step3').show();
}
...
function createPeer() {
  ...
  peer = new Peer(id,{ key: '62n3o1jhhi6kcsor', debug: 3, config: {'iceServers': [
  ...
  peer.on('error', function(err){
    if (err.type == 'unavailable-id') {
      //id = 'Animishmish';
      console.log("PEER CONN", peer.connections);
      peer.destroy();
      createPeer();
    }
  });
